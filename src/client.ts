...
export class Client extends events.EventEmitter {
   ...
   */
  public request(pcmd: any, pcfg: any, pcb: any, pcberr: any, ptype: string = "hash") {
    ...
    if (pcberr) {
      ...
      c.on("error", ( /*r*/ ) => {
      // console.error('ispapi-apiconnector: error event thrown in request method; no error cb method provided.');
   ...
   */
    public login(pparams: any, pcb: any, puri: string = "https://coreapi.1api.net/api/call.cgi", pcmdparams: any) {
   ...
   * @param {Function} pcb callback method
   */
  public logout(pcfg: any, pcb: any) {
    this.request({
      command: "EndSession",
    }, pcfg, pcb, pcb);
  }
   ...
   * @param {Object} pcfg the socket config
   */
  public createConnection(pcmd: any, pcfg: any) {
    let data = "";
