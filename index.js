...
ispapi.Client.prototype.connect = function(p_url){
	...
	this.socketcfg.agent = false;//disable socket pooling (avoids limitation to 5 sockets per host, or increase: maxSockets)
	if (!this.socketcfg.port){
		this.socketcfg.port = (this.socketcfg.protocol.match(/^https/i) ? '443' : '80');	
	}
...
ispapi.Client.prototype.headers = function(p_head){
	if (p_head && !p_head.hasOwnProperty('Expect')){
		p_head.Expect = '';
	}
...
ispapi.Client.prototype.request = function(p_cmd) {
	var key, data = "", req, oself = this;
	for (key in oself.logincfg){
		if (oself.logincfg.hasOwnProperty(key)){
			data += encodeURIComponent('s_' + key) + "=" + encodeURIComponent(oself.logincfg[key]) + "&";
		}
	}
	data += encodeURIComponent("s_command") + "=" + encodeURIComponent(ispapi.Client.command_encode(p_cmd));
...
/**
 * @method parse
 * @description convert the unparsed plain api response string to object notation 
...
/**
 * @method serialize
 * @description convert the parsed plain api response to an unparsed string based notation 
...
ispapi.Response.prototype = {
	/**
	 * @method useColumns
	 * @description sets the columns to be available in the response - if false it return all (default)
	 * @return {Object || null}
	 */
	useColumns : function(arr){
		...
		this.colregexp = (arr ? new RegExp("^(" + arr.join("|") + ")$", "i") : false);
	},
	/**
	 * @method rewind
	...
	/**
	 * @method hasNext
	 * @description returns true if the iterator can return a next row, false otherwise  
	 * @return {Boolean}
	 */
	hasNext : function(){
	...
	/**
	 * @method hasNext
	 * @description returns true if the iterator can return a previous row, false otherwise  
	 * @return {Boolean}
	 */
	hasPrevious : function(){
	...
	/**
	 * @method get
	 * @description returns the value of direct property of the parsed response object if found 
	 * @param {String} p_prop String specifying the property to fetch the value from
	 * @return {Object || String || Boolean}
	 *  Object if property found and is of type Object
	 *  String if property found and of type String (there are no further type cases!)
	 *  Boolean (false) if property not found 
	 */
	get : function(p_prop){
	 ...
	 * @description return the value of a given index of the given column/property identifier 
	 * @param {String} p_prop String specifying the column/property identifier
	 * @param {Integer} p_idx Integer specifying the index to fetch the value from
	 * @param {Boolean} p_cast_int Boolean specifying if the value should be casted to integer [optional]
	 * @return {String || Boolean} String in success case, Boolean (false) if failed 
	 */
	...
	as_string : function(){
		if (this.colregexp){
			return ispapi.Response.serialize(this.as_hash());
		}
	...
	as_hash : function(){
		if (this.colregexp){
			var d = JSON.parse(JSON.stringify(this.data.parsed)), key;
			if (d.hasOwnProperty("PROPERTY")){
				for (key in d.PROPERTY){
					if (d.PROPERTY.hasOwnProperty(key)){
						if (!key.match(this.colregexp)){
	...
	/**
	 * @method as_list
	 * @description return the parsed api response as list - better to use for realizing tables 
	 * @return {Object}
	 */
	as_list : function(){
		var r = this.as_hash(),
			tmp = {}, key, row, row2, i, count = 0;
		for (key in r){
				...
				for (i=0;i<count;i++){//run up to max index found
					...
					for (key in row){//run over all columns (properties) found
						if (row.hasOwnProperty(key) && r.PROPERTY[key][i]!==undefined){
							...
							//-- requires implementation of mechanisms to avoid access on these not existing indexes later (not part of this lib!)
	 ...
	 */
	columns : function(){
		var key, cols = [], props = this.properties();		
		if (props){
			for (key in props){
				if (props.hasOwnProperty(key) && !key.match(ispapi.Response.pagerRegexp)){
					cols.push(key);
				}
			}
		}
	...
	/**
	 * @method first
	 * @description return the number of the first response entry, used for pagination purposes 
	 * @return {Integer} 
	 */
	first : function(){
	...
	/**
	 * @method count
	 * @description return the count of items in the api list response, used for pagination purposes 
	 * @return {Integer} 
	 */
	count : function(){
		var c = this.getColumnIndex("COUNT", 0, true), cols, i, max = 0;
		if (c === false){
			c = 0;
			cols = this.columns();
	...
	/**
	 * @method last
	 * @description return the number of the last response entry, used for pagination purposes 
	 * @return {Integer} 
	 */
	last : function(){
	...
	/**
	 * @method limit
	 * @description return the number of items per page, used for pagination purposes 
	 * @return {Integer} 
	 */
	limit : function(){
	...
	/**
	 * @method total
	 * @description return the number of the total items matching the request in the backend, used for pagination purposes 
	 * @return {Integer} 
	 */
	total : function(){
	...
	/**
	 * @method pages
	 * @description return the count of result pages matching the request, used for pagination purposes 
	 * @return {Integer} 
	 */
	pages : function(){
		var t = this.total();
		if (t){
			return (Math.ceil(t / this.limit()) || 1);
		}
		return 1;
	},
	/**
	 * @method page
	 * @description return the current page number, used for pagination purposes 
	 * @return {Integer} 
	 */
	page : function(){
		if (this.count()){
			...
			if (limit){
				return Math.floor(this.first() / limit)+1;
			}
		}
		return 1;
	},
	/**
	 * @method prevpage
	 * @description return the previous page number, used for pagination purposes 
	 * @return {Integer} 
	 */
	prevpage : function(){
	...
	/**
	 * @method nextpage
	 * @description return the next page number, used for pagination purposes 
	 * @return {Integer} 
	 */
	nextpage : function(){
