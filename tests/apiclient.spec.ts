...
describe("APIClient class", function () {
  ...
  describe("#.saveSession/reuseSession", () => {
    ...
    it("validate correct settings", () => {
      const sessionobj = {};
      cl.setCredentials("myaccountid").setSession("12345678").saveSession(sessionobj);
      ...
      expect(tmp).to.equal(
        "s_login=myaccountid&s_sessionid=12345678&s_command=COMMAND%3DStatusAccount",
      );
    });
  });
  ...
  describe("#.logout", () => {
    it("validate against mocked API response [logout succeeded]", async () => {
      ...
      console.log("Logout Response:", cl.getSession());
