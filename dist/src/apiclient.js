...
export var ISPAPI_CONNECTION_URL_OTE =
...
var APIClient = (function () {
    ...
    if (!this.ua.length) {
      this.ua =
        "NODE-SDK ("
          .concat(process.platform, "; ")
          .concat(process.arch, "; rv:")
          .concat(this.getVersion(), ") ") + "node/".concat(process.version);
    }
    return this.ua;
  };
  ...
  APIClient.prototype.setRoleCredentials = function (uid, role, pw) {
    return this.setCredentials(
      role ? "".concat(uid, "!").concat(role) : uid,
      pw
    );
  };
    ...
    if (!(typeof cmd === "string" || cmd instanceof String)) {
      Object.keys(cmd).forEach(function (key) {
        if (cmd[key] !== null && cmd[key] !== undefined) {
          tmp += ""
            .concat(key, "=")
            .concat(cmd[key].toString().replace(/\r|\n/g, ""), "\n");
        }
      });
    } else {
      tmp = "" + cmd;
      ...
      if (val !== null && val !== undefined) {
        if (Array.isArray(val)) {
          ...
          for (var _i = 0, val_1 = val; _i < val_1.length; _i++) {
            var row = val_1[_i];
            newcmd["".concat(newKey).concat(index)] = (row + "").replace(
              /\r|\n/g,
              ""
            );
            index++;
